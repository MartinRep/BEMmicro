...
class OfferGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Offer entity")
        ...
        .repeat(2) {
            ...
            .post("/request/api/offers")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "description":"SAMPLE_TEXT", "availableOn":"2020-01-01T00:00:00.000Z", "price":null, "profile":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_offer_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created offer")
                .get("/request${new_offer_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created offer")
            .delete("/request${new_offer_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
